...
namespace FRASS.WebUI.Parcels
{
	public partial class Allotments : System.Web.UI.Page
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			{
				LoadModels();
				LoadPage(Convert.ToInt32(DropDownList_MarketModels.SelectedValue), Convert.ToInt32(DropDownList_RPA.SelectedValue));
			}
		}
		protected void Page_Init(object sender, EventArgs e)
		{
			...
			if (Request.QueryString["AllotmentNumber"] != null && Int32.TryParse(Request.QueryString["MarketModelPortfolioID"].ToString(), out tmpInt2))
			{
				allotmentNo = Request.QueryString["AllotmentNumber"].ToString();
				portfolio = dbDeliveredLogMarketModelDataManager.GetMarketModelPortfolio(tmpInt2);
			}
			else
			{
		...
		protected void LoadPage(int portfolioID, int rpaPortfolioID)
		{
		...
		{
			Response.Redirect("/PDFs/MarketModelAllotments.aspx?ParcelID=" + Request.QueryString["ParcelID"].ToString() + "&MarketModelPortfolioID=" + portfolio.MarketModelPortfolioID.ToString() + "&AllotmentNumber=" + allotmentNo.ToString(), true);
		}
		...
		private void LoadHarvestVolumesValueSummary(GridItem item)
		{
			...
			Repeater_HarvestData.DataSource = parcel.ParcelRiparians.Select(uu => uu.STD_ID).ToList<int>().Distinct();
